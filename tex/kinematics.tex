...
\begin{defn}
\define{Kinematics} is a complete description of a physical system at
any given moment.
\end{defn}
\begin{defn}
\define{Dynamics} describes the change of a physical system --- how and
...
\begin{defn}
The \define{Position Vector} for a point particle is a vector $\vec{r}$
...
\begin{defn}
The \define{Velocity Vector} (or just \emph{``Velocity''}) for a point
...
\begin{defn}
The \define{Average Velocity} for a point particle over a time interval
...
\input{tex/kinematics/avg-velocity-along-string}
...
\N{Parallel Acceleration}\index{Acceleration!Parallel}
...
\N{Perpendicular Acceleration}\label{chunk:perpAcceleration}\index{Acceleration!Perpendicular}
...
\input{tex/kinematics/uniform-car}

\subsection{Projectile Motion}
\N{Definition}
A \define{Projectile}\index{Projectile|textbf} is any body that is given an initial velocity,
...
called its \define{Trajectory}\index{Trajectory!for Projectile}.
...
\N{Parabolic Motion}\index{Projectile!Parabolic Motion}
...
\N[Napoleon 12-Pounder]{Example}\index{Napoleon 12-Pounder}
...
motion is called \define{Uniform Circular Motion}\index{Circular Motion!Uniform}.

\N{Derivation of Acceleration}\label{M:kinematics:uniform-circular-motion:derivation-of-acceleration}
...
\N*{Lemma} \textit{The triangles in both these figures are similar.}
...
towards the circle's center, and is called \define{Centripetal Acceleration}\index{Acceleration!Centripetal}\index{Centripetal Acceleration}
...
\define{Period}\index{Period!for Circular Motion}
...
\define{Non-Uniform Circular Motion}\index{Circular Motion!Non-Uniform}.
\end{definition}
\M
We see that the derivation of acceleration in uniform circular motion
\Mref{M:kinematics:uniform-circular-motion:derivation-of-acceleration}
...
\begin{definition}
We also have the \define{Tangent Acceleration}\index{Tangent Acceleration}\index{Acceleration!Tangent} which describes the
...
relative to that observer, or simply \define{Relative
  Velocity}\index{Relative Velocity|textbf}\index{Velocity!Relative}.
\end{definition}

\begin{definition}
An observer, equipped with (in principle) a meter stick and a stopwatch,
forms a \define{Frame of Reference}\index{Reference Frame|textbf} (or a ``\textit{Reference Frame\/}'').
\end{definition}
\begin{remark}
