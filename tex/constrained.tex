...
momenta. The conditions $\phi_{m}(q,p)=0$ are called \define{Primary
  Constraints}\index{Constraint!Primary|textbf}\index{Primary Constraint|textbf} to emphasize the equations of motion \emph{are not} used
to obtain these relations, and they imply no restriction on the
coordinates $q^{n}$ and their velocities $\dot{q}^{n}$.

\begin{definition}
The \define{Primary Constraint Surface}\index{Primary
Constraint!Surface}\index{Primary Constraint
Surface|textbf}\index{Constraint Surface!Primary} consists of the submanifold
...
\N{Regularity Conditions}\index{Regularity Conditions|(}\label{n:constrained-hamiltonian:regularity-cond}
...
immediately.\index{Regularity Conditions|)}
\end{proof}
...
\begin{theorem}\label{thm:variation-tangent-to-constraint-surface}
...
the \define{Canonical
Hamiltonian}\index{Hamiltonian!Canonical}\index{Canonical Hamiltonian|textbf}
\begin{equation}
H = \dot{q}^{n}p_{n} - L.
\end{equation}
\end{definition}
...
&=\dot{q}^{n}\delta p_{n}+p_{n}\delta \dot{q}^{n}
  -\left(\frac{\partial L}{\partial q^{n}}\delta q^{n}
         + \frac{\partial L}{\partial\dot{q}^{n}}\delta\dot{q}^{n}\right)\\
&=\dot{q}^{n}\delta p_{n}
   +\left(p_{n}-\frac{\partial L}{\partial\dot{q}^{n}}\right)\delta\dot{q}^{n}
  -\frac{\partial L}{\partial q^{n}}\delta q^{n}\\
&=\dot{q}^{n}\delta p_{n} -\frac{\partial L}{\partial q^{n}}\delta q^{n}.
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{equation}
\delta H - \left(\dot{q}^{n}\delta p_{n} -\frac{\partial L}{\partial q^{n}}\delta q^{n}\right) = 0
\end{equation}
...
- \dot{q}^{n}\delta p_{n} + \frac{\partial L}{\partial q^{n}}\delta q^{n}=0
\end{equation}
...
\N{Derived from Variational Principle}\label{M:total-action-principle}
...
\dot{p}_{n} = -\frac{\partial H}{\partial q^{n}}
  -u^{m}\frac{\partial\phi_{m}}{\partial q^{n}}.
\end{equation}
Observe\marginpar{{\footnotesize\em $u^{m}$ as Lagrange multipliers}}
then that the $u^{m}$ now serve as Lagrange multipliers\index{Lagrange Multipliers!in Gauge Systems}
...
first solve the constraints then perform the variation
\begin{equation}
...
\N{Consistency of Constraints}\index{Constraint!Consistency}
...
\begin{definition}
A \define{Secondary
Constraint}\index{Constraint!Secondary}\index{Secondary Constraint|textbf} is a relation of the $p$'s and $q$'s
...
Again, this either (i) creates new constraints [aptly called ``Tertiary
  Constraints''\index{Tertiary Constraint}\index{Constraint!Tertiary} by some authors], or (ii) restricts the
...
\begin{definition}
We introduce \define{Weak Equality}\index{Equality!Weak, for Constrained Systems}\index{Weak Equality|textbf}
...
\define{Reducible}\index{Reducible Constraints|textbf}\index{Constraint!Reducible} (or \emph{``Redundant''}) and say we are in the
reducible case. If the constraints are independent, then they are
\define{Irreducible}\index{Irreducible Constraints|textbf}\index{Constraint!Irreducible}.
\end{definition}
...
\end{theorem}
(The $\firstClassHamiltonian$ is called the \define{First-Class Hamiltonian}\index{Hamiltonian!First-Class}\index{First-Class Hamiltonian|textbf}, c.f. \Mref{defn:constrained:first-class}.)
\begin{proof}
We see explicitly
\begin{equation}
  \begin{split}
...
\begin{definition}\label{defn:total-hamiltonian}
  The \define{Total Hamiltonian}\index{Hamiltonian!Total}\index{Total Hamiltonian|textbf} is
  \begin{equation}
...
\define{First-Class}\index{First-Class Function!On Phase Space} iff its Poisson bracket with every constraint
weakly vanishes
\begin{equation}
...
A function is called \define{Second-Class}\index{Second-Class Function!On Phase Space} iff it is not first-class.
\end{definition}
...
\begin{theorem}\label{thm:constrained:phi-a-complete-first-class-primary-constraints}\index{First-Class Constraints!Complete Set for Primary ---}
...
\begin{remark}[Definition of Physical State]\index{Physical State!of Gauge System|(}
...
arbitrariness we discovered in constrained systems.\index{Physical State!of Gauge System|)}
\end{remark}
...
Transformation}\index{Gauge Transformation} iff
\begin{equation}
...
Transformations.]\index{Gauge Transformation!Composing}
...
\N{First-Class Secondary Constraints As Gauge
Generators?}\index{First-Class Constraints!As Gauge Generators}
...
\N{Dirac's Conjecture}\index{Dirac Conjecture!for Gauge Systems}
...
\N{Caution: Generally Covariant Systems}\index{General Covariance!As Gauge}
...
\N[{Allcock~\cite{allcock1975}}]{Example}\index{Dirac Conjecture!Counter-Example}
...
\N{Notation} We denote all first-class constraints as
...
the total Hamiltonian \Mref{defn:total-hamiltonian}, i.e., first-class primary
constraints\dots which is not natural! We thus introduce a ``natural''
generalization.

\begin{definition}\label{defn:extended-hamiltonian}
The \define{Extended
Hamiltonian}\index{Hamiltonian!Extended}\index{Extended Hamiltonian|textbf} is
\begin{equation*}
...
\M
Recall the total action principle \Mref{M:total-action-principle}, we had
\begin{equation*}
...
\section{Secondary Constraints and the Dirac Bracket}
...
\begin{definition}
The \define{Dirac Bracket}\index{Dirac Bracket|textbf} is
\begin{equation}
...
\begin{theorem}\label{thm:dirac-bracket:addlProperties}
...
\M\label{M:dirac-bracket:dynamics}\index{Dirac Bracket!Dynamics}
The extended Hamiltonian \Mref{defn:extended-hamiltonian}, being
...
\M\label{M:dirac-bracket:gauge-transformations}\index{Dirac Bracket!Gauge Transformations}\index{Gauge Transformation!and Dirac Bracket}
...
\N{Eliminating Second-Class Constraints}\index{Second-Class Constraint!Elimination}
All the equations of the theory areformulated in terms of the Dirac
bracket. We see this works for dynamics \Mref{M:dirac-bracket:dynamics}
and gauge
transformations \Mref{M:dirac-bracket:gauge-transformations}. Then the
...
\N{Remark}
The separation of constraints into first-class and second-class is not
