...
\end{thm}
(The $H'$ is called the \define{First-Class Hamiltonian}, c.f. \Mref{defn:constrained:first-class}.)
\begin{proof}
We see explicitly
\begin{equation}
  \begin{split}
...
\begin{defn}
  ...
  \begin{equation}
    \totalHamiltonian \eqDef H' + v^{a}\phi_{a} = H + u^{m}\phi_{m}
  \end{equation}
...
\N{Non-Uniqueness of $H'$, $\phi_{a}$}
This split of $\totalHamiltonian$ into $H'$ and $v^{a}\phi_{a}$ is not
unique due to the arbitrariness of $U^{m}$. That is to say, we could
pick a different solution $\widetilde{U}^{m}$ and get a diffferent split. By
the completeness of $\phi_{a}$ \Mref{thm:constrained:phi-a-complete-first-class-primary-constraints}, we see this amounts to picking different
arbitrary functions $\widetilde{v}^{a}\neq v^{a}$, giving us
\begin{equation}
\totalHamiltonian = \widetilde{H}' + \widetilde{v}^{a}\phi_{a}
\end{equation}
...
&\weakEq\left.\left(F + \PoissonBracket{F}{H'}\delta t + \PoissonBracket{F}{\phi_{a}}v^{a}\delta{t}\right)\right|_{t_{1}}
\end{split}
\end{equation}
...
\begin{lemma}
The Poisson bracket $\PoissonBracket{\phi_{a}}{H'}$ of any first-class
primary constraint $\phi_{a}$ with the first-class Hamiltonian generates
...
\intertext{and}
\delta F_{2} &= \varepsilon\PoissonBracket{F+\delta F_{1}}{H'}\\
&=\varepsilon\PoissonBracket{F}{H'} + \varepsilon\eta^{a}\PoissonBracket{\PoissonBracket{F}{\phi_{a}}}{H'}
\end{align}
\end{subequations}
but if we first evolve, 
\begin{subequations}
\begin{align}
\delta F_{1}' &= \varepsilon\PoissonBracket{F}{H'}\\
\intertext{then perform a gauge transformation}
\delta F_{2}' &= \eta^{a}\PoissonBracket{F+\delta F_{1}'}{\phi_{a}}\\
&= \eta^{a}\PoissonBracket{F}{\phi_{a}}+\varepsilon\eta^{a}\PoissonBracket{\PoissonBracket{F}{H'}}{\phi_{a}}.
\end{align}
\end{subequations}
We claim
\begin{subequations}
...
&= \varepsilon\eta^{a}(\PoissonBracket{\PoissonBracket{F}{\phi_{a}}}{H'}
-\PoissonBracket{\PoissonBracket{F}{H'}}{\phi_{a}})\\
&= \varepsilon\eta^{a}\PoissonBracket{F}{\PoissonBracket{\phi_{a}}{H'}}
\end{align}
\end{subequations}
...
since $\phi_{a}$ and $H'$ are first-class, the Poisson bracket
$\PoissonBracket{\phi_{a}}{\phi_{a'}}$ and
$\PoissonBracket{\phi_{a'}}{H'}$ are also first-class, so their linear
combinations are first-class. But there is no reason to expect their
linear combination to contain only primary constraints. In practice,
most first-class secondary constraints come about in this manner.
...
We argued $\phi_{a}$ and $\PoissonBracket{\phi_{a}}{H'}$ generate
transformations that do not alter the physical state, while implicitly
